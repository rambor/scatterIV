...
//        System.out.println("distribution sizes " + sphericalDistribution.getItemCount() + " == " + prDistribution.getItemCount());
     ...
     * normalize to volume
     */
    public void normalizeToSphericalDistribution(){
        // find largest value in experimental distribution
        int indexMax =0;
        double testvalue=Double.NEGATIVE_INFINITY;
        for(int i=1; i<prDistribution.getItemCount(); i++){
            XYDataItem value = prDistribution.getDataItem(i);
            if (value.getYValue() > testvalue){
                testvalue = value.getYValue();
                indexMax = i;
            }
        }

        // make scale factor that fully contains P(r)-distribution
        double scaleFactor = sphericalDistribution.getY(indexMax).doubleValue()/testvalue;

        // test if any values are exceed spherical distribution in rescale
        ...
        while(continueIt){
            ...
            for(int i=1; i<prDistribution.getItemCount(); i++){
                XYDataItem value = prDistribution.getDataItem(i);
                prDistribution.updateByIndex(i, value.getYValue()*scaleFactor); // rescale
                value = prDistribution.getDataItem(i);
                if (value.getYValue() > sphericalDistribution.getY(i).doubleValue()){
                    testvalue = value.getYValue();
                    indexMax = i;
                    continueIt = true;
                }
            ...
            if (continueIt){ // new scale factor
                   scaleFactor = sphericalDistribution.getY(indexMax).doubleValue()/testvalue;
            }
            round += 1;
        }

        double area = 0, spharea=0, rgSphere=0, averageR = 0, sphereAvgR=0, modelRg=0;
        for(int i=1; i<prDistribution.getItemCount(); i++){
            XYDataItem value = prDistribution.getDataItem(i);
            if (value.getYValue() > sphericalDistribution.getY(i).doubleValue()){
                System.out.println("TOO BIG IN distribution " + value.getYValue() + " " + sphericalDistribution.getY(i).doubleValue());
            }

            area += value.getYValue();
            modelRg += value.getYValue()*value.getXValue()*value.getXValue();
            averageR += value.getXValue()*value.getYValue();
            spharea += sphericalDistribution.getY(i).doubleValue();
            sphereAvgR += sphericalDistribution.getY(i).doubleValue()*value.getXValue();
            rgSphere += value.getXValue()*value.getXValue()*sphericalDistribution.getY(i).doubleValue();
            //System.out.println(value.getX() + " " + value.getYValue() + " " + sphericalDistribution.getY(i));
        }

        averageR /= area;

        double rgS = Math.sqrt(0.5*rgSphere/spharea);
        modelRg = Math.sqrt(0.5*modelRg/area);

        double upperFractionP = 1.0, keptFraction = upperFractionP;
        double decrement = 0.993;

        double diffRg = Double.POSITIVE_INFINITY, modelarea, frac, diffarea, tempdiff, tempRg, tempAverageR;
        double keptarea = 0.0;

        System.out.println("Sphere :: rg " + rgS + " frac " + Constants.FourDecPlace.format(area/spharea) + " Initial_Vol " + Constants.TwoDecPlace.format(area/spharea*sphericalVolume) + " " +  Constants.TwoDecPlace.format(36/35*dmax*0.5));
        System.out.println("Area :: " + Constants.OneDecPlace.format(spharea) + " " + Constants.OneDecPlace.format(spharea*bin_width) + " " + Constants.TwoDecPlace.format(modelRg));

        int counter = 1;
        double bestCompRg=0.0, bestRpp=0.0, bestDiff = Double.POSITIVE_INFINITY, bestFrac = modelRg/area, rgCompdiff = Double.NEGATIVE_INFINITY;

        while (upperFractionP > 0.01){

            // calculate area of remaining part
            tempAverageR = 0.0;
            diffarea = 0.0;
            tempRg = 0.0;
            modelarea = 0.0;
            for(int i=1; i<prDistribution.getItemCount(); i++){
                XYDataItem value = prDistribution.getDataItem(i);
                frac = upperFractionP*value.getYValue();
                tempdiff = sphericalDistribution.getY(i).doubleValue() - frac;
                diffarea += tempdiff;
                modelarea += frac;
                tempAverageR += value.getXValue()*tempdiff;
                tempRg += value.getXValue()*value.getXValue()*tempdiff;
                //System.out.println(value.getX() + " " + value.getYValue() + " " + sphericalDistribution.getY(i));
            }

            // for given fraction what are the Rg_complement and r-values

            double wtFrac = modelarea/spharea;
            double target = (rgS*rgS - wtFrac*(modelRg*modelRg))/(1.0-wtFrac);
            tempRg = 0.5*tempRg/diffarea;
            double putativeRg = Math.sqrt(tempRg), temprgDiff, temprg2;

            if (rg < sphericalRg){ // rg_complement must be greater
                double radius = dmax*0.5;
                double startRg = sphericalRg;
                double upperLimit = Math.sqrt(2.0/3.0)*radius + 0.1*Math.sqrt(2.0/3.0)*radius;

                while (startRg < upperLimit){

                    double rpp = 0.01;
                    temprgDiff = startRg - putativeRg;
                    temprg2 = temprgDiff*temprgDiff;
                    while (rpp < radius){
                        tempdiff = target - (startRg*startRg + wtFrac*rpp*rpp);
                        if (tempdiff < bestDiff && tempdiff > 0 ){
                            bestDiff = tempdiff;
                            rgCompdiff = temprg2;
                            bestCompRg = startRg;
                            bestRpp = rpp;
                            bestFrac = wtFrac;
                            System.out.println(wtFrac + " best Comp Rg " + bestCompRg + " best rpp " + bestRpp + " " + (bestFrac*sphericalVolume) + " " + Math.sqrt(tempRg));
                        }
                        rpp += 0.1;
                    }
                    startRg += 0.02;
            ...
            tempdiff = averageR - tempAverageR/diffarea;

//            double tempV = rgS*rgS - (modelarea/spharea*modelRg*modelRg + (1.0 - modelarea/spharea)*tempRg + modelarea/spharea*(1.0-modelarea/spharea)*tempdiff*tempdiff);
            double rval = (rgS*rgS - (modelarea/spharea*modelRg*modelRg + (1.0 - modelarea/spharea)*tempRg))/(modelarea/spharea*(1.0-modelarea/spharea));

//            System.out.println(Constants.OneDecPlace.format(sphericalRg*sphericalRg) + " " + Constants.OneDecPlace.format(modelarea/spharea*rg*rg) + " " + Constants.OneDecPlace.format((1.0 - modelarea/spharea)*tempRg) + " :: " + Constants.FourDecPlace.format(modelarea/spharea) + " -- " + Constants.TwoDecPlace.format(rval));
//            double rootval = Math.sqrt(sphericalRg*sphericalRg - (rg*rg + tempRg));
//            System.out.println(Constants.TwoDecPlace.format(tempV) + " fraction " + Constants.FourDecPlace.format(modelarea/spharea) + " + " + Constants.FourDecPlace.format(diffarea/spharea) + " V: " + Constants.OneDecPlace.format(modelarea/spharea*sphericalVolume) + " SphRg " + Constants.TwoDecPlace.format(rgS) + " tempRg " + Constants.TwoDecPlace.format(Math.sqrt(tempRg))  + " rave " );
//            System.out.println(upperFractionP + " " + (modelarea/spharea*sphericalVolume)+ " " + Math.sqrt(rval));

            if (rval < diffRg && rval > 0){
                diffRg = rval;
                keptFraction = modelarea/spharea;
                keptarea = modelarea;
//                System.out.println(counter +
//                        " fraction " + Constants.FourDecPlace.format(modelarea/spharea) +
//                        " + " + Constants.FourDecPlace.format(diffarea/spharea) +
//                        " V: " + Constants.OneDecPlace.format(modelarea/spharea*sphericalVolume) +
//                        " modelRg " + Constants.TwoDecPlace.format(modelRg) + " " + Constants.TwoDecPlace.format(rg) +
//                        " rave " + Constants.Scientific1.format(Math.sqrt(rval)));
            }

            counter++;
            upperFractionP *= decrement;
        }
...
//        for(int i=1; i<prDistribution.getItemCount(); i++){
//            XYDataItem value = prDistribution.getDataItem(i);
//            System.out.println(value.getX() + " " + keptFraction*value.getYValue() + " " + sphericalDistribution.getY(i));
//        }

        // iterate through upperFractionP
        System.out.println(round + " area :: " + (modelRg/area*bin_width) + " :: " + (bestFrac*sphericalVolume) + " diff :: " + diffRg + " rg " + rg);
    }

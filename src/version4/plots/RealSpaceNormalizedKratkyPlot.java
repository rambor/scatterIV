...
public class RealSpaceNormalizedKratkyPlot extends ScatterPlot {
    private final ValueMarker yMarker = new ValueMarker(1.1);
    private final ValueMarker xMarker = new ValueMarker(1.7320508);

    public RealSpaceNormalizedKratkyPlot(Collection collection, WorkingDirectory wkd) {
        super(collection, wkd);

        locationOfWindow = new Point(225,300);
        frame = new ChartFrame("SC\u212BTTER \u2263 Real-Space Dimensionless Kratky PLOT", chart);
        JPopupMenu popup = frame.getChartPanel().getPopupMenu();
        popup.add(toggler);

        domainAxis = new NumberAxis("qRg");
        rangeAxis = new NumberAxis("Normalized Intensity");
        String quote = "q\u2217Rg";
        ...
        if (evt.getPropertyName() == "mainCollection") {
            ...
            if (frame.isVisible()){
                ...
                for (int i = 0; i < totalSets; i++){
                    ...
                    if (temp.getInUse()){
                        if (temp.getNormalizedKratkyRealSpaceRgData().getItemCount() == 0){
                            temp.createNormalizedRealSpaceKratky();
                        }
                        plottedDatasets.addSeries(temp.getNormalizedKratkyRealSpaceRgData());  // should add an empty Series
                    }
                }
    ...
    public void plot() {
        ...
        for (int i = 0; i < totalSets; i++){
            ...
            if (temp.getInUse()){
                temp.createNormalizedRealSpaceKratky();
                plottedDatasets.addSeries(temp.getNormalizedKratkyRealSpaceRgData());
            }
        }
